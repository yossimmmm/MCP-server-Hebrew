...
const STT_RECOGNIZER = process.env.SPEECH_V2_RECOGNIZER_WIDGET ??
    process.env.SPEECH_V2_RECOGNIZER ??
    "";
if (!STT_RECOGNIZER) {
    console.warn("[widget-voice] SPEECH_V2_RECOGNIZER is not set");
}
export default function attachWidgetVoiceWs(server) {
            ...
            state.stt = createHebrewChirp3Stream(STT_RECOGNIZER, {
                ...
                async onData(text, isFinal) {
                    try {
                        ...
                        console.error("[widget-voice] LLM/handler error:", e?.message || e);
