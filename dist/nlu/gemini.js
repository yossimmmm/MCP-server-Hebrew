...
export class LlmSession {
    ...
    constructor() {
        ...
        if (!this.apiKey) {
            console.error("[LLM] GEMINI_API_KEY is not set – running in fallback mode without Gemini calls");
        }
        else {
    ...
    buildPrompt(userText) {
        ...
        if (WAITING_PHRASES.length > 0) {
            const waitingList = WAITING_PHRASES.map((p, idx) => `${idx + 1}. "${p.text}"`).join("\n");
            lines.push("אלה משפטי ההמתנה המוקלטים שאתה יכול לבחור מהם ב-waiting_hint (אל תמציא חדשים):");
            lines.push(waitingList);
            lines.push("");
        }
        // קטלוג מוצרים מודבק (אם קיים)
        if (PRODUCT_CATALOG_RAW.trim()) {
            lines.push("קטלוג המוצרים המלא שעומד לרשותך (אל תשנה את הטקסט, רק תשתמש בו כמידע להמלצות):");
        ...
        lines.push(userText);
        lines.push("");
        lines.push("ענה אך ורק ב-JSON תקין עם השדות reply, memory, waiting_hint בלבד, ללא טקסט נוסף, ובלי ```.");
        return lines.join("\n");
    }
    async reply(userText) {
        ...
        if (!this.apiKey) {
            const fallback = "יש לי כרגע תקלה בחיבור למנוע החכם, אבל אני איתך. תנסה להסביר שוב בפשטות מה אתה צריך.";
        ...
        try {
            ...
            let newWaitingHint = this.waitingHint ?? null;
            ...
            try {
                ...
                if (typeof parsed.waiting_hint === "string" &&
                ...
                const waitingCandidate = this.extractFieldFromRaw(jsonText, "waiting_hint");
                if (waitingCandidate) {
