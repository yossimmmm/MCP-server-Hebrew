...
export interface WidgetSession {
  id: string;
  llm: LlmSession;
  createdAt: number; // last access time
}
...
// ---- session management ----
export function getOrCreateSession(sessionId?: string | null): WidgetSession {
  const now = Date.now();
  ...
  const session: WidgetSession = { id, llm, createdAt: now };
...
export function attachWidgetRoutes(app: Express): void {
  // JS bundle
  ...
  // LLM + TTS endpoint
  app.post("/api/widget/message", async (req: Request, res: Response) => {
    try {
