...
export function attachTwilioWs(server: http.Server) {
  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    function maybeBargeIn(partial: string) {
      if (!partial || !speaking) return;
      const norm = partial.replace(/\s+/g, " ").trim();
      if (norm.length >= 6) cancelCurrent("stt-partial");
    }
      ...
      onFinal: (text) => {
        (async () => {
          try {
            ...
            for (const phrase of splitForPhone(reply)) enqueue(phrase);
          } catch (err: any) {
    ...
    ws.on("message", (buf) => {
      ...
      switch (msg.event) {
        ...
        case "media":
          if ((msg as any).media?.payload) {
            // inbound μ-law base64 → STT
            stt.writeMuLaw((msg as any).media.payload);
          }
          break;
        ...
        case "stop":
          ...
          try { stt.end(); } catch {}
          try { cancelCurrent("ws-stop"); } catch {}
          try { ws.close(); } catch {}
          break;
      }
    });
    ...
    const cleanup = () => {
      try { stt.end(); } catch {}
      try { cancelCurrent("ws-close"); } catch {}
...
function splitForPhone(s: string): string[] {
